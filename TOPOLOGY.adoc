...
parts of its system architecture diagram. The canonical _current_
...
|*L1* |Region capabilities + capability environment threading
|`+formal/TypingL1.v+` + `+formal/Semantics_L1.v+` |judgment 100%,
semantics 15 Qed / 9 admits (L2-integration debt)

|*L2* |Structural modality (Linear vs Affine) |`+formal/Modality.v+` +
`+m : Modality+` in `+has_type_l1+` |core landed, `+linear_to_affine+`
Qed with zero axioms

|*L3* |Echo / residue (irreversibility evidence) |`+formal/Echo.v+` +
upstream `+hyperpolymath/echo-types+` |calculus done (12 Qed), wiring
into typing pending

|*L4* |Dyadic interaction semantics |(none yet) |design in
`+PRESERVATION-DESIGN.md §7+`

|🛑 |Legacy `+formal/Semantics.v+` + `+formal/Typing.v+` |archaeology
|preservation provably false — see `+formal/Counterexample.v+`
|===
____
