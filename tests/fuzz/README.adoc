...
This directory contains fuzz testing harnesses for Ephapax using
cargo-fuzz.

=== Current Status

* ✅ Property-based tests exist (proptest in
`+src/ephapax-cli/tests/property_tests.rs+`)
* ❌ No real fuzz testing implemented yet
* The `+placeholder.txt+` file was a scorecard requirement but provides
no actual fuzzing
...
* *Goal*: Find parser crashes, infinite loops, or incorrect AST
generation
...
Start with a seed corpus of: - Valid Ephapax programs from `+examples/+`
- Invalid programs that currently fail gracefully - Edge cases (empty
files, very large numbers, deep nesting)
