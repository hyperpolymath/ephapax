...
This guide explains how to use the fuzz testing infrastructure for
Ephapax.
...
This will: 1. Install cargo-fuzz if needed 2. Build the fuzz targets 3.
Run both fuzzers for 60 seconds each
...
[arabic]
. *Run in isolated environments*: Fuzzers can consume significant
resources
. *Set time limits*: Always use `+-max_total_time+`
. *Monitor resource usage*: Fuzzing can use lots of CPU and memory
. *Start with small inputs*: Use `+-max_len=1024+` initially
. *Review crashes carefully*: Not all crashes are security issues

=== Troubleshooting

==== "`No targets specified`"

Make sure you’re in the `+tests/fuzz+` directory and have run
`+cargo fuzz build+`.
